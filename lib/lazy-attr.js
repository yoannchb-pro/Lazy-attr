...
window.lazyDatas = {
    ...
    updateURL: "https://unpkg.com/lazy-attr@latest/lazy-attr.min.js",
...
window.lazy = function(){
    ...
    return {
        ...
        version: "1.1.2",
        //version matcher
        versionMatcher: "[#version]1.1.2[#version]" 
    }
}
    ...
    window.lazy().getLastVersion(function(v){
        if(v.error){
            ...
            try{
                ...
                if(nv) console.info("Lazzy-attr : new version " + ver + " available !");
                if(good) console.info("Lazy-attr : you have the latest version " + actv)
                if(error) console.info("Lazzy-attr : " + actv + " is not a valid version !");
            } catch(e) {}
        }
    });
});
